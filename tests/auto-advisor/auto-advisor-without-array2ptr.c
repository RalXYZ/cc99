...
{
    char buf[1000];
    while (scanf("%s", &buf[0]) != -1)
    {
        int index1 = find_char_in_range(&buf[0], '|', 0, strlen_my(&buf[0]));
        ...
        if (index3 != index2 + 1)
        {
            // a trick, QAQ
            buf[index3] = ';';
            while ((semicolon = find_char_in_range(&buf[0], ';', semicolon + 1, index3 + 1)) != -1)
            {
                ...
                for (i = last_semicolon + 1; i < semicolon; i++)
                {
                    if (buf[i] == ',')
                    {
                        strcpy_my(&courses_pre_course[courses_num][pre_status_num][pre_course_num][0], &buf[last_semicolon + 1],
                                  i - last_semicolon - 1);
                        pre_course_num++;
                        last_semicolon = i;
                    }
            ...
            else
            {
                for (int j = 0; j < courses_pre_course_num[i]; j++)
                {
                    ...
                    while (courses_pre_course[i][j][pre_num][0] != '\0')
                    {
                        ...
                        {
                            if (strcmp_my(&courses_pre_course[i][j][pre_num][0], &courses_name[k][0]) == 0)
                            {
